# Changelog

All notable changes to this project will be documented in this file.

The format is based on [Keep a Changelog](https://keepachangelog.com/en/1.1.0/), and this project follows [Semantic Versioning](https://semver.org/spec/v2.0.0.html).

## [0.1.0] - 2026-06-17

### Added

- TypeScript accessibility crawler powered by Appium and WebdriverIO.
- Self-hosted Next.js dashboard for configuring and running crawls.
- iOS Simulator onboarding for device, runtime, and Expo Dev Client bundle id.
- Optional Appium, simulator, and Expo startup hooks.
- Timestamped report history with delete/open actions.
- Screenshot, XML, JSON, and Markdown report artifacts.
- Screen-level accessibility findings and evidence preview.
- Grouped crawler events for automation diagnostics.
- Light/dark mode with system-theme default.
- English and Turkish dashboard UI.
